...
## 🚘 Download the Driving Algorithms
...
## ⚙️ Modify the Pure Pursuit Node Settings
...
After launching, use the **“2D Pose Estimate”** button in RViz to set the vehicle’s approximate initial pose on the map.
...
* In `pure_pursuit_node.py`, the `is_real` flag must be set correctly.
  ...
  * `True`  → real F1Tenth AV
  * `False` → simulation mode
* You **must** set the initial particle filter pose using the RViz tool **“2D Pose Estimate”**.
